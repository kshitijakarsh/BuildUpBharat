  ...
  <nav className="flex items-center justify-between px-4 md:px-12 py-4 bg-white border-b border-gray-100 sticky top-0 z-50 gap-6">
    ...
    <div className="flex items-center gap-4 md:gap-8">
      ...
      <div className="relative p-2 cursor-pointer hover:bg-gray-50 rounded-full transition-colors">
        ...
        <span className="absolute top-2.5 right-2.5 w-2.5 h-2.5 bg-[#FF6B6B] border-2 border-white rounded-full"></span>
      </div>

      <div className="hidden md:block h-10 w-px bg-gray-200"></div>

      {/* User Profile */}
      <Link to="/profile" className="flex items-center gap-3 bg-[#F8F9FF] border border-[#E9EFFF] rounded-full pl-5 cursor-pointer hover:border-brand-blue/30 transition-all">
        <span className="text-sm font-semibold text-brand-blue-text whitespace-nowrap hidden sm:block">
          Aryan Sharma
        </span>
        <div className="w-9 h-9 md:w-10 md:h-10 rounded-full overflow-hidden border-2 border-brand-blue-start">
          <img
